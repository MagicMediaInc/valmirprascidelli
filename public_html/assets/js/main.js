$(function(){
	...
	$(".candidato").each(function(e){
		nome = "nome="+$(this).children(".nome").text();
		contenido = $(this).html();
		cod = $(this).children(".cod").text();
		$(this).html("<a href='coladigital/"+ cod +"' style='text-decoration:none; color:#000;' >" + contenido + "</a>")
		// img = "&img="+$(this).find('img').attr('src');
		// console.log($(this).find('img').attr('src'));
		// datos = "'" + $(this).children(".cod").text() + "'" + "=> array( 'nome'=>" + $(this).children(".nome").text() + ", 'img'=>" + $(this).find('img').attr('src') + "),";
		// alert("'" + $(this).children(".cod").text() + "'" + "=> array(/n 'nome'=>" + $(this).children(".nome").text() + ", /n 'img'=>" + $(this).find('img').attr('src') + "),");
		// $(".php").append(datos);
	})
	$("#loading").hide();
	...
	$("#candidato").on("submit",function(e){
		e.preventDefault();
		datos=$(this).serialize();
		if(datos.img!=""){
		$.ajax({
			...
			error:function(e) {
				...
				$("#response").html("Nothing to send! ").show(300).delay(2000).fadeOut(900);
